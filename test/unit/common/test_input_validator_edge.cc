...
TEST(InputValidatorEdgeTest, DevicePathEdges) {
  // Path not starting with / and not COM/Special (e.g. relative path or custom name)
...
TEST(InputValidatorEdgeTest, PositiveNumberEdges) {
  // Max int64
  EXPECT_NO_THROW(
      InputValidator::validate_positive_number(std::numeric_limits<int64_t>::max(), "max_int64"));
}
