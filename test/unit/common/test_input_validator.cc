...
TEST(InputValidatorTest, DetailedHelperLogic) {
  ...
  EXPECT_THROW(InputValidator::validate_host("example.-com"), diagnostics::ValidationException);  // Label starts with hyphen
  EXPECT_THROW(InputValidator::validate_host("example.com-"), diagnostics::ValidationException);  // Ends with hyphen (caught by back() check too)
