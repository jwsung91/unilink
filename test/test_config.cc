...
TEST_F(ConfigTest, ConfigValidationBoundaryValues) {
    ...
    auto client1 = UnifiedBuilder::tcp_client("127.0.0.1", 1)
        .auto_start(false)
        .build();
    ...
    auto client2 = UnifiedBuilder::tcp_client("127.0.0.1", 65535)
        .auto_start(false)
        .build();
...
TEST_F(ConfigTest, ConfigValidationInvalidValuesNetwork) {
    ...
    auto client = UnifiedBuilder::tcp_client("127.0.0.1", 0)
        .auto_start(false)
        .build();
    ...
    auto client2 = UnifiedBuilder::tcp_client("", test_port_)
        .auto_start(false)
        .build();
...
TEST_F(ConfigTest, ConfigChangeNotificationsMultipleKeys) {
    ...
    std::cout << "Multiple key change notifications working: " << change_count.load() << " notifications received" << std::endl;
}
...
TEST_F(ConfigTest, ConfigThreadSafetyMixedOperations) {
    ...
    EXPECT_EQ(completed_operations.load(), num_threads * operations_per_thread);
    std::cout << "Mixed operations thread safety test completed: " << completed_operations.load() << " operations" << std::endl;
}
...
TEST_F(ConfigTest, ConfigValidationAndErrorHandling) {
    ...
    std::cout << "Non-existent key validation: " << (nonexistent_validation_result.is_valid ? "valid" : "invalid") << std::endl;
