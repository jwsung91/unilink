...
TEST_F(MockIntegratedTest, DependencyInjectionSetup) {
    ...
    injector.registerSocketFactory("test", []() {
        return std::make_unique<MockTcpSocket>();
    });
    
    injector.registerAcceptorFactory("test", []() {
        return std::make_unique<MockTcpAcceptor>();
    });
    
    injector.registerSerialFactory("test", []() {
...
TEST_F(MockIntegratedTest, MockPerformanceTest) {
    ...
    std::cout << "Created " << socket_count << " mock sockets in " 
