...
TEST_F(SerialTest, SerialDifferentBaudRates) {
    ...
    for (auto baud_rate : test_baud_rates_) {
        auto serial = UnifiedBuilder::serial("/dev/ttyUSB0", baud_rate)
            .auto_start(false)
            .build();
...
TEST_F(SerialTest, SerialNonExistentDevice) {
    ...
    auto serial = UnifiedBuilder::serial(non_existent_device, 9600)
        .auto_start(false)
        .build();
...
TEST_F(SerialTest, SerialExtremeBaudRates) {
    ...
    std::vector<uint32_t> extreme_baud_rates = {
        50, 75, 110, 134, 150, 200, 300, 600, 1200, 1800, 2400, 4800,
        14400, 28800, 38400, 57600, 115200, 230400, 460800, 921600,
...
TEST_F(SerialTest, SerialInvalidDevicePaths) {
    ...
    std::vector<std::string> invalid_paths = {
        "", "invalid", "/dev/invalid", "/dev/tty", "/dev/ttyX", 
        "COM", "COMX", "LPT1", "PRN", "AUX", "NUL",
        "/dev/null", "/dev/zero", "/dev/random", "/dev/urandom"
    };
    
    for (const auto& path : invalid_paths) {
        auto serial = UnifiedBuilder::serial(path, 9600)
            .auto_start(false)
            .build();
...
TEST_F(SerialTest, SerialSpecialCharactersInDevicePath) {
    ...
    std::vector<std::string> special_paths = {
        ...
        "/dev/ttyUSB0}close",
        "/dev/ttyUSB0|pipe",
        "/dev/ttyUSB0\\backslash",
        "/dev/ttyUSB0:colon",
        "/dev/ttyUSB0;semcolon",
        "/dev/ttyUSB0\"quote",
        "/dev/ttyUSB0'apostrophe",
        "/dev/ttyUSB0<less",
        "/dev/ttyUSB0>greater",
        "/dev/ttyUSB0,comma",
        "/dev/ttyUSB0.question",
        "/dev/ttyUSB0/slash",
        "/dev/ttyUSB0 space",
        "/dev/ttyUSB0\ttab",
        "/dev/ttyUSB0\nnewline",
...
TEST_F(SerialTest, SerialErrorHandling) {
    ...
    auto serial = UnifiedBuilder::serial("/dev/ttyUSB0", 9600)
        ...
        .on_error([](const std::string& error) {
            std::cout << "Error callback triggered: " << error << std::endl;
        })
        .build();
...
TEST_F(SerialTest, SerialConcurrentOperations) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            auto serial = UnifiedBuilder::serial("/dev/ttyUSB" + std::to_string(t), 9600)
                .auto_start(false)
                .build();
