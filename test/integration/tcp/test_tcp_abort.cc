...
protected:
    void SetUp() override {
        unilink::test::NetworkTest::SetUp();
    }
 ...
 * Verifies server handles client sending RST gracefully while data might be pending or partial.
 */
TEST_F(TcpAbortTest, SessionAbortion) {
    ...
    server_ = unilink::tcp_server(test_port_)
        .unlimited_clients()
        .on_multi_disconnect([&disconnected](size_t){
            disconnected = true;
        })
        .on_error([&error_reported](const std::string& err){
            // Depending on implementation, RST might trigger on_error or just on_disconnect
            // Usually read error (connection reset by peer) triggers on_disconnect.
            // But if it was unexpected during read, maybe error logged.
            // Let's just track if it crashes or not.
            error_reported = true;
        })
        .build();
    ...
    try {
        socket.connect(tcp::endpoint(net::ip::address::from_string("127.0.0.1"), test_port_));
    ...
    bool closed_gracefully = unilink::test::TestUtils::waitForCondition([&disconnected](){
    ...
    try {
        socket2.connect(tcp::endpoint(net::ip::address::from_string("127.0.0.1"), test_port_));
