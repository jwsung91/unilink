...
public:
     ...
     */
    TestableTcpClient(const std::string& host, uint16_t port) 
        : wrapper::TcpClient(host, port) {
     ...
     */
    MockTcpSocket* getMockSocket() const {
        return mock_socket_.get();
    }
...
public:
     ...
     */
    MockTcpAcceptor* getMockAcceptor() const {
        return mock_acceptor_.get();
    }
