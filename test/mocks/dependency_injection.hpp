...
public:
     ...
     */
    void registerSocketFactory(const std::string& key, SocketFactory factory) {
        socket_factories_[key] = factory;
    }

    /**
     * @brief Register mock acceptor factory
     */
    void registerAcceptorFactory(const std::string& key, AcceptorFactory factory) {
        acceptor_factories_[key] = factory;
    }

    /**
     * @brief Register mock serial factory
     */
    void registerSerialFactory(const std::string& key, SerialFactory factory) {
        serial_factories_[key] = factory;
    }
...
public:
    MockTestScope() {
        DependencyInjector::instance().setTestingMode(true);
    }
    
    ~MockTestScope() {
        DependencyInjector::instance().clear();
    }
