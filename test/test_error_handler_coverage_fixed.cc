...
TEST_F(ErrorHandlerTest, ConnectionErrorReporting) {
    ...
    common::error_reporting::report_connection_error(
        component, operation, ec, is_retryable);
...
TEST_F(ErrorHandlerTest, CommunicationErrorReporting) {
    ...
    common::error_reporting::report_communication_error(
        component, operation, error_message, is_retryable);
...
TEST_F(ErrorHandlerTest, ConfigurationErrorReporting) {
    ...
    common::error_reporting::report_configuration_error(
...
TEST_F(ErrorHandlerTest, MemoryErrorReporting) {
    ...
    common::error_reporting::report_memory_error(
        component, operation, error_message);
...
TEST_F(ErrorHandlerTest, SystemErrorReporting) {
    ...
    common::error_reporting::report_system_error(
        component, operation, error_message, ec);
...
TEST_F(ErrorHandlerTest, ErrorRateCalculation) {
    ...
    for (int i = 0; i < 10; ++i) {
        common::error_reporting::report_connection_error(
            "client", "connect", boost::system::error_code{}, true);
...
TEST_F(ErrorHandlerTest, ErrorCallbackRegistration) {
    ...
    auto callback = [&callback_count, &last_callback_error](
        const common::ErrorInfo& error_info) {
        callback_count++;
    ...
    EXPECT_TRUE(TestUtils::waitForCondition([&callback_count]() {
        return callback_count.load() > 0;
    }, 1000));
...
TEST_F(ErrorHandlerTest, ErrorCallbackWithLevels) {
    ...
    EXPECT_TRUE(TestUtils::waitForCondition([&received_levels]() {
        return received_levels.size() >= 2;
    }, 1000));
...
TEST_F(ErrorHandlerTest, ErrorThresholdDetection) {
    ...
    for (int i = 0; i < 5; ++i) {
        common::error_reporting::report_connection_error(
            "client", "connect", boost::system::error_code{}, false);
    }
...
TEST_F(ErrorHandlerTest, ErrorHandlerReset) {
    ...
    std::atomic<int> callback_count{0};
    auto callback = [&callback_count](const common::ErrorInfo&) {
        callback_count++;
    };
    error_handler.register_callback(callback);
