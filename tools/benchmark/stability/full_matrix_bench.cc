...
struct LoadParam { size_t payload_size; microseconds sleep_us; };
...
struct BenchResult { double tp; double dr; };
...
BenchResult run_tcp(int level, unilink::base::constants::BackpressureStrategy strategy) {
    ...
    std::atomic<bool> run{true}; std::atomic<uint64_t> sb{0}; std::string p(load.payload_size, 'A');
    std::thread sender([&]{
        while(run){ if(client.send(p)) sb += p.size(); if(load.sleep_us.count()>0) std::this_thread::sleep_for(load.sleep_us); }
    });
    std::this_thread::sleep_for(seconds(3)); run = false; sender.join();
...
BenchResult run_uds(int level, unilink::base::constants::BackpressureStrategy strategy) {
    auto load = get_load(level);
    std::string path = "/tmp/full_uds_" + std::to_string(level) + "_" + std::to_string(static_cast<int>(strategy)) + ".sock";
    fs::remove(path);
    ...
    std::this_thread::sleep_for(seconds(3)); run = false; sender.join();
    server.stop(); client.stop(); fs::remove(path);
    return {(sb/(1024.0*1024.0))/3.0, (sb>0 ? (100.0*rb/sb) : 0.0)};
}
...
BenchResult run_udp(int level, unilink::base::constants::BackpressureStrategy strategy) {
    ...
    server.on_data([&](const unilink::MessageContext& ctx){ rb += ctx.data().size(); });
    (void)server.start_sync();
    unilink::config::UdpConfig cfg; cfg.local_port = pc; cfg.remote_address = "127.0.0.1"; cfg.remote_port = ps;
    unilink::UdpClient client(cfg);
    client.backpressure_strategy(strategy);
    (void)client.start_sync();
    std::atomic<bool> run{true}; std::atomic<uint64_t> sb{0}; 
...
int main() {
    ...
    auto run_all = [](std::string name, auto func) {
        for(int l=1; l<=4; ++l) {
            ...
            std::cout << "| " << std::left << std::setw(5) << name << " | L" << l << " | Reliable | " << std::fixed << std::setprecision(2) << std::setw(17) << r.tp << " | " << std::setw(12) << r.dr << " |" << std::endl;
            auto b = func(l, unilink::base::constants::BackpressureStrategy::BestEffort);
            std::cout << "| " << std::left << std::setw(5) << name << " | L" << l << " | BestEffort | " << std::fixed << std::setprecision(2) << std::setw(15) << b.tp << " | " << std::setw(12) << b.dr << " |" << std::endl;
        }
    };
    run_all("TCP", run_tcp);
    run_all("UDS", run_uds);
