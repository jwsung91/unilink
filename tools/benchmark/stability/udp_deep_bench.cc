...
BenchResult run_udp_eval(int level, unilink::base::constants::BackpressureStrategy strategy) {
    ...
    client.backpressure_threshold(16 * 1024 * 1024); // 16MB
    client.backpressure_strategy(strategy);
    if (!client.start_sync()) { server.stop(); return {0, 0}; }
...
int main() {
    ...
    for (int l = 1; l <= 4; ++l) {
        ...
        std::cout << "| L" << l << " | Reliable | " << std::fixed << std::setprecision(2) << std::setw(17) << r.tp << " | " << std::setw(12) << r.dr << " |" << std::endl;
        
        auto b = run_udp_eval(l, unilink::base::constants::BackpressureStrategy::BestEffort);
        std::cout << "| L" << l << " | BestEffort | " << std::fixed << std::setprecision(2) << std::setw(15) << b.tp << " | " << std::setw(12) << b.dr << " |" << std::endl;
    }
    return 0;
}
