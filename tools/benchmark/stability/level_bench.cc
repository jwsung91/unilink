...
BenchResult run_single_bench(int level, unilink::base::constants::BackpressureStrategy strategy) {
  ...
  server.on_data(
      [&](const unilink::MessageContext& ctx) { recv_bytes.fetch_add(ctx.data().size(), std::memory_order_relaxed); });
  server.start_sync();
...
int main() {
  ...
  for (int l = 1; l <= 4; ++l) {
    ...
    std::cout << "| L" << l << " | Reliable | " << std::fixed << std::setprecision(2) << std::setw(17)
              << res_r.throughput_mb_s << " | " << std::setw(12) << res_r.delivery_rate << " |" << std::endl;

    auto res_b = run_single_bench(l, unilink::base::constants::BackpressureStrategy::BestEffort);
    std::cout << "| L" << l << " | BestEffort | " << std::fixed << std::setprecision(2) << std::setw(15)
              << res_b.throughput_mb_s << " | " << std::setw(12) << res_b.delivery_rate << " |" << std::endl;
  }
  return 0;
}
