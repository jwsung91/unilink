...
| Metric | KeepAll (16MB) + Flow Control | **KeepLatest (1MB) + No FC** | Performance Gain |
|:---|:---:|:---:|:---:|
...
UDS highlights the cost of context switching and blocking in Python. `KeepAll` with Flow Control forced the Python thread to wait for I/O completion frequently, resulting in only 80 MB/s. 
By switching to `KeepLatest` and removing Flow Control, the Python sender was **never blocked**. It successfully pushed data into the Unilink C++ core at over **1.3 GB/s**, effectively saturating the local IPC bus.

### 2. Extreme Freshness (Bufferbloat Prevention)
During network interruptions, `KeepLatest` flushed the queue **40,899 times**. With a 1MB threshold, this ensures that the receiver never deals with more than 1MB of "old" data. In a 1.3 GB/s stream, 1MB represents less than **1ms of latency**. 
`KeepAll` would have buffered 16MB or more, leading to hundreds of milliseconds of stale data once the connection recovered.
