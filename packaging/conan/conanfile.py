...
class UnilinkConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.settings.os == "Linux":
            ...
            if (
                ...
                self.settings.compiler.version
                and Version(self.settings.compiler.version) < "13.0"
            ):
    ...
    def package(self):
        ...
        copy(
            self,
            "*.hpp",
            src=os.path.join(self.source_folder, "unilink"),
            dst=os.path.join(self.package_folder, "include", "unilink"),
        )
        copy(
            self,
            "*.h",
        ...
        copy(
            ...
            "unilinkConfig.cmake",
            src=self.build_folder,
            dst=os.path.join(self.package_folder, "lib", "cmake", "unilink"),
        )
        copy(
            self,
            "unilinkConfigVersion.cmake",
            src=self.build_folder,
            dst=os.path.join(self.package_folder, "lib", "cmake", "unilink"),
        )
        copy(
            self,
            "unilinkTargets.cmake",
            src=self.build_folder,
            dst=os.path.join(self.package_folder, "lib", "cmake", "unilink"),
        )
        copy(
        ...
        if self.options.build_docs:
            copy(
                self,
                "*",
                src=os.path.join(self.source_folder, "docs", "html"),
                dst=os.path.join(
                    self.package_folder, "share", "doc", "unilink", "html"
                ),
            )
        ...
        if self.options.build_examples:
            copy(
                ...
                dst=os.path.join(self.package_folder, "share", "unilink", "examples"),
            )
    ...
    def package_info(self):
        ...
        self.cpp_info.components["unilink"].set_property(
            "cmake_target_name", "unilink::unilink"
        )
