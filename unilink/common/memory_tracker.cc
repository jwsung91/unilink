...
MemoryTracker::MemoryStats MemoryTracker::get_stats() const {
    return stats_;
}
...
void MemoryTracker::disable_tracking() {
    tracking_enabled_.store(false);
}

bool MemoryTracker::is_tracking_enabled() const {
    return tracking_enabled_.load();
}
...
void MemoryTracker::print_leak_report() const {
    ...
    for (const auto& alloc : leaked_allocations) {
        std::cout << "Leaked: " << alloc.size << " bytes at " << alloc.ptr 
                  << " allocated in " << alloc.file << ":" << alloc.line 
                  << " (" << alloc.function << ")" << std::endl;
...
void ScopedMemoryTracker::track_deallocation(void* ptr) {
