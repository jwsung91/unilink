...
void ErrorHandler::set_min_error_level(ErrorLevel level) {
    min_level_.store(level);
}

ErrorLevel ErrorHandler::get_min_error_level() const {
...
bool ErrorHandler::is_enabled() const {
...
void ErrorHandler::add_to_recent_errors(const ErrorInfo& error) {
    ...
    if (recent_errors_.size() > MAX_RECENT_ERRORS) {
        recent_errors_.erase(recent_errors_.begin(), 
                           recent_errors_.begin() + (recent_errors_.size() - MAX_RECENT_ERRORS));
    }
}
...
void report_connection_error(const std::string& component, const std::string& operation,
    ...
    ErrorInfo error(ErrorLevel::ERROR, ErrorCategory::CONNECTION, component, operation,
                   ec.message(), ec, retryable);
    ErrorHandler::instance().report_error(error);
}

void report_communication_error(const std::string& component, const std::string& operation,
...
void report_memory_error(const std::string& component, const std::string& operation,
                        const std::string& message) {
    ErrorInfo error(ErrorLevel::CRITICAL, ErrorCategory::MEMORY, component, operation, message);
    ErrorHandler::instance().report_error(error);
}

void report_system_error(const std::string& component, const std::string& operation,
                        const std::string& message, const boost::system::error_code& ec) {
    ErrorInfo error(ErrorLevel::ERROR, ErrorCategory::SYSTEM, component, operation, message, ec);
    ErrorHandler::instance().report_error(error);
}

void report_warning(const std::string& component, const std::string& operation,
                   const std::string& message) {
    ErrorInfo error(ErrorLevel::WARNING, ErrorCategory::UNKNOWN, component, operation, message);
    ErrorHandler::instance().report_error(error);
}

void report_info(const std::string& component, const std::string& operation,
                const std::string& message) {
